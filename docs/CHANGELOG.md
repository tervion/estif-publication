...
Entries are reverse-chronological; backdated entries are marked as such.

---

## [6.4.2] — 2026-07-21 — Repository restructure, path repairs, documentation consolidated to four files

### Changed — tests/ reorganized to the three-folder layout (RHAC-011)
- `tests/` now contains exactly `scripts/` (all 85 .py, with `phase2_a1prime/`
  kept whole inside), `docs/` (TEST_INDEX.md + all cached data .txt), and
  `plots/`. Zero loose files (end-state receipt 6/6 PASS).
- `tests/_index_view/` deleted — redundant with `tests/docs/TEST_INDEX.md`, the
  single source of truth. `src/files.zip` deleted.
- All path constructs repointed in one idempotent pass (63 edits across 54
  files; second run: 0 edits): sys.path hops recomputed depth-aware for the new
  layout; every CACHE_DIR now targets `tests/docs` (uniform policy — scripts/
  stays pure .py, caches never duplicate beside scripts).

### Fixed — dead container paths (pre-existing breakage, not reorg fallout)
- `test_desi_wz_consistency.py` and `test_sparc_bias_analysis.py` hard-coded
  `/home/claude/...` cache paths from an old container session and could not run
  on the author's machine at all; `test_sparc_tully_fisher.py` carried two inert
  fallbacks of the same kind. All repaired to repo-relative `tests/docs/` paths.
- The two DESI readers now share one cache set (`dr1_mean.txt` family); the two
  SPARC scripts converge on one VOTable cache (`SPARC_Lelli2016c.xml` — run
  `test_sparc_tully_fisher.py` once to download it, then
  `test_sparc_bias_analysis.py` reads the same file).
- `test_tilt_models.py` reached `src/` by leaving the repository and re-entering
  through the parent folder's literal name; now a clean in-repo hop.

### Changed — documentation consolidated 7+ → 4 (nothing dropped)
- Living set: `docs/PROJECT.md` (status + checklist + roadmap + structure),
  `docs/SCIENCE.md` (framework + evidence), `docs/RHAC.md` (decision archive,
  moved from `docs/plan/`), `CHANGELOG.md` (this file). Root `README.md` slimmed
  to a landing page.
- Absorbed and superseded (queued by the author for archive/deletion):
  STATUS.md, ROADMAP.md and PATH_ONE_CHECKLIST.md → PROJECT.md;
  ESTIF_CONCEPT.md, VALIDATION_REPORT.md, SUMMARY_FOR_REVIEW.md,
  GAZTANAGA_COMPARISON.md, JWST_TEST_SPEC.md, PHASE2_DECLARATION.md and the
  13 Jul doctrine memo (ESTIF_document_update_guide.md) → SCIENCE.md;
  CORRECTIONS_v6.3.1.md and MILESTONE_v6.3_THE_SPLIT.md → already recorded
  below as [6.3.1] and [6.3.0]; PROGRESS_REPORT_2026-07-09.md → its period is
  covered by [6.3.0]–[6.3.2].
- The rename batch owed since RHAC-006 (taboo terms; "pending NAMING.md") is
  delivered at the document layer by this consolidation; no NAMING.md needed.
  Script-internal prose untouched (future pass).

### Added — RHAC-011; receipt canon; reconciliations
- RHAC-011 records the restructure and consolidation; summary statistics
  recounted and enumerated (17 resolved/closed · 1 conditional · 4 active ·
  3 standing · 11 decision records).
- Receipt canon: UKN pair at `archive/6-4-2 diagnostics/`; door suite +
  re-census (PASS 5/5) + growth pair at `tests/scripts/phase2_a1prime/`;
  fronts 1–3, C-15, P-derivation and the four a₀-doctrine scripts at
  `tests/scripts/` (RHAC-008's 13 Jul locate-note resolved).
- Checklist reconciliations: B-7 executed 11 Jul (see [6.3.3] below);
  B-4b partial (Gaztañaga verdict delivered; item-4 numerical cross-check
  owed); Scenario O writing task completed in the v6.3 concept rewrite.
...
## [6.3.3] — 2026-07-11 — Letter measurement session: target lock, lensing BTFR, a₀ tension, μ extraction (backdated entry, recorded 21 Jul)

*The 11 July working session that fed the v6.3 → v6.4 letter ran alongside the
Phase-2 closure but was never logged. Recorded here from the receipts.*

### Added — Path Two target lock (pre-registration)
- `estif_pathtwo_target_lock.py`: fits the CPL thawing shape w(a) = w₀ + wₐ(1−a)
  on the exact Path One DESI DR2 pipeline (same bins, RD = 147.09 and H₀ fixed,
  Ωm fixed at x₀) to freeze the χ²/N ≈ 0.66 bullseye from runtime — before any
  derivation could aim at it. Receipt: `tests/docs/pathtwo_target_lock_output.txt`.
  Same-day Phase 2 closed Path Two (honorable null), so the locked target now
  stands as the pre-registered bar any *outside-the-axioms* evolving-w proposal
  must beat.

### Added — weak-lensing BTFR vs the derived a₀ (checklist B-7)
- `btfr_lensing.py`: Mistele, McGaugh, Lelli, Schombert & Li 2024 (ApJL 969 L3)
  lensing BTFR against a₀ = 1.192×10⁻¹⁰ m/s², zero free parameters. Predictions
  run ~5–13% low; χ²/N ≈ 1.5 with the 0.1 dex correlated M*/L systematic
  (5.6–8.0 stat-only) — consistent within systematics, degenerate with the
  stellar-mass calibration.
- `a0_tension_corrected.py`: the implied-a₀ inversion with the M*/L systematic
  treated as fully correlated across bins (the earlier per-bin √N dilution was
  an error): kinematic +1.17σ, lensing +0.69σ to +2.04σ (ETG worst) above the
  derived value — mild tension, stated plainly.

### Added — μ extraction (Reading A vs Reading B)
- `mu_extraction.py`: differentiating the dynamic-n force law locally
  (Reading A) sends g_obs/g_N → 0 in the galactic range x ~ 10⁻⁶–10⁻⁸, the
  opposite of MOND's rising μ; Newton requires n frozen at ½ (Reading B). The
  receipt behind RHAC-001's finding that μ(a/a₀) is underived and no rule fixes
  where n is evaluated.

### Notes
- Scripts live at `tests/scripts/` since the [6.4.2] restructure; the archive
  audit copies are under `archive/6-4-2 diagnostics/audit/Jul 11 Session/`.
