...
which is mathematically IDENTICAL to LCDM. Two honest consequences:
 ...
 (2) ESTIF adds a geometric CONSISTENCY RELATION LCDM lacks: Omega_m = x0 =
     R_H/r_universe (0.12% from Planck). NOTE: r_universe is the LCDM particle
     horizon, which contains Omega_m, so this is a consistency check, not yet an
     independent prediction -- it becomes one only when r_universe is derived
     within ESTIF.
...
def main():
    ...
    print(f"  Omega_m (ESTIF = x0) = {OMEGA_M_ESTIF:.4f}   [geometric prediction]")
    ...
    if abs(c2_core - c2_lcdm) < 0.1:
        ...
        print("    - Omega_m = x0 is a CONSISTENCY RELATION (0.12%), not yet an")
        print("      independent prediction: r_universe is the LCDM horizon, which")
        print("      itself contains Omega_m. Genuine prediction pending an internal")
        print("      ESTIF derivation of r_universe;")
